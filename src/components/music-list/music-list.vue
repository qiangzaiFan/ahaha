...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted() {
      this.imageHeight = this.$refs.bgStyle.clientHeight    //记录背景图片的高度
      this.mintranslateY = -this.imageHeight+TOP_HEIGHT  //假如我们可以滚动到顶部,滚动的最大距离
    ...
    watch: {
      scrollY(newY) {
        let translateY = Math.max(this.mintranslateY,newY)  //最远滚动距离不会超过背景图片的高度
        let zIndex = 0
        let scale = 1
        let blur = 0
        // debugger
        ...
        if(newY< this.mintranslateY){
          ...
          this.$refs.bgStyle.style.height = `${TOP_HEIGHT}px`
